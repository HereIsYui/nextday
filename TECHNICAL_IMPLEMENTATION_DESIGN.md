...
| 端 | 定位 | 首版范围 |
| --- | --- | --- |
| 鱼排聊天室右侧插件 | 轻量伴随入口 | 查看修行状态、领取基础收益、提交预设行动、查看任务提醒、跳转 Web/H5 |
...
| 鱼排插件 | TypeScript 编译为单 JS + CSS，注入鱼排右侧面板 |
...
- 插件复用 `game-client` 和少量 `ui`，不引入完整 Web 应用。
...
| 功能 | 说明 |
| --- | --- |
| 状态卡片 | 境界、修为、离线收益、行动令、今日任务进度 |
| 一键领取 | 领取离线收益、洞府基础收益、日常奖励 |
| 一键提交 | 按预设提交九塔默认行动或普通探索 |
| 提醒 | 行动令满、可突破、月卡赠抽未用、任务可领 |
| 快捷入口 | 打开 Web/H5 的九州、任务、抽卡、月卡页面 |
| 消息提示 | 成功、失败、风控、需登录、需跳转 |
...
- 鱼排插件能在聊天室右侧展示状态、一键领取、提交预设行动和跳转 Web。
- 三端使用同一套游戏 API 和账号体系。
- 付费、抽卡、战斗、奖励均由服务端结算。
- 鱼排积分订单重复回调不会重复到账。
