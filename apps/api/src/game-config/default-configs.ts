...
export const defaultConfigEnvelopes: Record<string, ConfigEnvelope> = {
  ...
  pill: {
    config_type: "pill",
    config_version: productionConfigVersion,
    ruleset_version: productionFormulaRuleVersion,
    reward_config_version: productionRewardConfigVersion,
    payload: {
      crafting_mode: "player_discovery",
      craftable_materials: getProductionCraftMaterials("alchemy"),
      recipe_visibility: "不公开默认药方；仅展示可投炉丹材和玩家已保存的单方。",
  ...
  forge: {
    ...
    payload: {
      crafting_mode: "player_discovery",
      craftable_materials: getProductionCraftMaterials("forge"),
      recipe_visibility: "不公开默认器方；材料组合决定法宝类型、品阶倾向和词条池。",
