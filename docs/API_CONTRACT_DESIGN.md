...
| `TerritoryNode` | 地块内可占领节点，如资源点、灵脉、关隘、州府、九塔 |
| `OccupationRecord` | 占领、易主、放弃、保护和收益权变化记录 |
...
- 客户端只提交行军、占领、建造、驻防等意图；战斗、占领结果、资源产出和州战积分必须由服务端结算。
- 主城不可永久易主；主城失败只能进入城防破损、围困、附庸、免战恢复等可恢复状态。
- 分城、资源点、灵脉、关隘和九塔奇观可以真实易主。
- 所有状态变更必须幂等，重复请求不能重复扣资源、重复创建队列或重复占领。
- 付费、月卡、VIP 不得提高占领倍率、行军胜率、州战积分倍率和地图结算倍率。
