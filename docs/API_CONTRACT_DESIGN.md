...
> **历史说明（2026-08-04）**：本文含有旧城池、领地、地图、军队、行军、围城、州战、PVP 与 `recipe_id` 配方接口描述，均仅为历史记录，不可作为现行实现依据。现行文字玩法与接口以 [《文字修行玩法说明》](TEXT_CULTIVATION_DESIGN.md) 及实际共享契约为准。

> **现行行动规则（2026-08-10）**：探索已经收敛为长期行动。`POST /api/game/explore` 只能启动长期探索，`POST /api/game/actions/start` 只能启动长期修炼或长期探索，探索请求不接受 `count`；在线按小时批量结算，离线最多计算 8 小时并通过 `/api/game/actions/offline-reward/claim` 领取。`/api/game/explore/current`、`/api/game/explore/claim` 和 `/api/game/daily-route` 已退役，今日路线、下一步推荐和按次数探索不得作为客户端入口。
...
| `/api/plugin/submit-preset` | POST | 插件预设提交：九塔行动、宗门任务等白名单行动；不再提供按次数探索预设 |
...
## 十五、P3 接口占位与玩法深度边界（历史归档）

以下内容记录旧 P3 设计，保留用于版本追溯。现行实现不提供今日路线，也不按次数完成普通探索；当前探索结算、离线快照和战报字段以共享类型及《现有玩法闭环修复计划》为准。P3 的奖励、掉落、战斗胜负、技能学习和材料消耗边界仍然有效。
...
| 今日路线（已退役） | `/api/game/daily-route` | GET | 历史接口；现行服务返回 404，不得重新接入 |
...
- 现行长期行动只能由玩家主动选择州域或修炼，不能由客户端根据“下一步”推荐自动启动；不能绕过行动令、月卡权益、风控、幂等和结算规则。
...
- P3 探索、战报、生产和技能接口只能增强玩法深度和展示解释，不改变付费隔离、奖励预算、服务端结算和异步参与边界；今日路线不属于现行接口。
