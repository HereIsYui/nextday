...
> **历史说明（2026-08-04）**：本文含有旧城池、领地、地图、军队、行军、围城、州战、PVP 与 `recipe_id` 固定配方的数据表描述，均仅为历史记录，不可作为现行实现依据。现行数据库结构以 Prisma Schema、已执行迁移及 [《文字修行玩法说明》](TEXT_CULTIVATION_DESIGN.md) 为准。
