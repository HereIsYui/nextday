...
> **历史说明（2026-08-04）**：本文含有旧城池、领地、地图、军队、行军、围城、州战、PVP 与 `recipe_id` 固定配方的数据模型描述，均仅为历史记录，不可作为现行实现依据。现行文字玩法数据模型以 Prisma Schema、共享契约及 [《文字修行玩法说明》](TEXT_CULTIVATION_DESIGN.md) 为准。
