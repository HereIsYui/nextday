...
- 一套后端服务支撑三端。
- Web 和 H5 共享主要前端代码。
- 鱼排插件默认展示小状态卡片，点击后展开复杂功能窗口，不复制完整游戏。
...
| 端 | 定位 | 首版范围 |
| --- | --- | --- |
| 鱼排聊天室右侧插件 | 小卡片默认态 + 复杂窗口展开态 | 默认展示当前状态和少量快捷操作；点击后展开修行、今日任务、洞府轻操作、九州/九塔摘要、预设行动、简化战报、PVP 提醒、古宝赠抽提醒、宗门提醒、跳转 Web/H5 |
...
插件默认态只负责“看一眼当前状态”和少量快捷操作；展开态负责 30 分钟日常中 60%-70% 的高频轻操作，完整体验由 Web/H5 承接。
...
- 所有端只调用游戏 API。
- 当前 MVP 三端通讯以 HTTP REST 风格 API 为主，查询使用 `GET`，操作提交使用 `POST`。
- 鱼排积分扣减必须由服务端发起。
...
| 通讯方式 | 当前定位 | 适用内容 |
| --- | --- | --- |
| HTTP GET | MVP 默认 | 角色状态、插件小卡片、任务摘要、九州 / 九塔摘要、战报历史、配置和公告 |
| HTTP POST | MVP 默认 | 行动提交、奖励领取、炼丹、炼器、抽卡、月卡购买、订单创建、宗门操作 |
| WebSocket / SSE | 后续增强 | 红点变化、行动结算完成、战报生成、邮件公告、月卡赠抽提醒 |

HTTP 是当前唯一默认交互通道。所有状态变更接口必须携带幂等键，所有奖励、抽卡、战斗、订单和贡献结果都由服务端结算。WebSocket / SSE 即使后续接入，也只用于提醒和轻量状态推送，不参与核心结算；客户端收到推送后仍通过 HTTP 拉取权威状态。
...
| 功能 | 说明 |
| --- | --- |
| 当前状态 | 境界、修为进度、离线收益、行动令 |
| 今日提醒 | 可领取、可突破、月卡赠抽未用、行动令接近上限 |
| 快捷操作 | 一键领取基础收益、打开复杂窗口、跳转 Web/H5 |
...
fishpi-plugin/
  ...
  src/
    mount.ts        创建容器并挂载应用
    components/     小状态卡、展开窗口、任务、九塔、提醒组件
    api/            插件专用 API SDK 包装
...
- 默认态必须保持为小卡片，不默认展开复杂面板，不长期遮挡聊天室右侧主要区域。
...
- 插件 UI 的复杂功能只能在用户点击后展开，必须控制体积、请求频率和渲染成本，不影响鱼排聊天室主体验。
- 插件失败时降级为跳转 Web/H5。
...
当前 P3 后续阶段优先打磨桌面 Web。H5/PWA 暂不继续复用压缩后的桌面布局，等桌面今日路线、事务抽屉、成长闭环和战报体验稳定后，作为独立移动端专项重新设计。
...
- 插件默认小状态卡片、展开式复杂窗口、日课聚合、九塔预设、洞府轻操作、战报提醒、跳转入口。
- H5 移动导航和 PWA。
- 插件和 H5 共享 API SDK。
...
- 鱼排插件能在聊天室右侧以小状态卡片展示当前状态，点击后展开复杂窗口查看修行、任务、洞府、九州、九塔、战报、古宝和宗门提醒，并完成一键领取与预设行动提交。
- 三端使用同一套游戏 API 和账号体系。
- 付费、抽卡、战斗、奖励均由服务端结算。
- 鱼排积分订单重复回调不会重复到账。
